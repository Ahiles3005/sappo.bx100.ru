...
$this->addExternalJs(SITE_TEMPLATE_PATH.'/js/maska.js');
	...
	<form action="<?=POST_FORM_ACTION_URI?>" method="POST" name="ORDER_FORM" id="bx-soa-order-form" enctype="multipart/form-data">
		...
		<div id="bx-soa-order" class="row orderform--v1 bx-<?=$arParams['TEMPLATE_THEME']?>" style="opacity: 0">
			...
			<div class="col-sm-9 bx-soa">
				...
				<div class="pandd grid-sbonus">
					<? if ($arParams['DELIVERY_TO_PAYSYSTEM'] === 'p2d'): ?>
						<!--	PAY SYSTEMS BLOCK	-->
						<div id="bx-soa-paysystem" data-visited="false" class="bx-soa-section bx-active  v2">
							<div class="bx-soa-section-title-container">
								<h2 class="bx-soa-section-title col-sm-9">
					...
					<? else: ?>
						...
						<!--	PAY SYSTEMS BLOCK	-->
						<div id="bx-soa-paysystem" data-visited="false" class="bx-soa-section bx-active v1">
							<div class="bx-soa-section-title-container">
								<h2 class="bx-soa-section-title col-sm-9">
									<span class="bx-soa-section-title-count"></span><?=$arParams['MESS_PAYMENT_BLOCK_NAME']?>
								</h2>
...
<?php
                        ...
                        if(\Bitrix\Main\Engine\CurrentUser::get()->getId()) {
                            ...
                            if($userBonuses->getClientId()) {
                                if( $userBonuses->getClientBonuses()->maxBonusOut > 0) {
                                    $bonusBalance = $userBonuses->getClientBonuses()->bonusBalance;
                                    //   $bonyses = $this->user->getClientBonuses()->bonusBalance; ?>
                                    <!--	PICKUP BLOCK	-->
                                    <div id="bx-soa-bonus" data-visited="false" class="bx-soa-section order-bonuses" >

                                        <div class="bx-soa-section-content container-fluid">
                                            <div class="order-bonuses__title">
                                                Списать бонусы

                                                <div class="sbonus" style="margin-left: auto"><?= $bonusBalance; ?></div>
                                            </div>

                                            <div class="order-bonuses__body">
                                                <div class="order-bonuses__info">
                                                    У вас <?= $bonusBalance ?> баллов.<br>
                                                    В этом заказе можно списать <?= $userBonuses->getClientBonuses()->maxBonusOut ?> балла.<br>
													<div class="order-bonuses__warn">При списании бонусы не начисляются!</div>
                                                </div>
                                                <input type="number" class="order-bonuses__input" id="orderBonusesVal" value="0" min="0" max="<?= $userBonuses->getClientBonuses()->maxBonusOut ?>">

                                                <div class="order-bonuses__slider-container">
                                                    <div class="order-bonuses__slider-wrap">
                                                        <input type="range" min="0" max="<?= $userBonuses->getClientBonuses()->maxBonusOut ?>" value="0" class="order-bonuses__slider" id="orderBonusesRange">
                                                    </div>
                                                    <div class="min-max">
                                                        <div class="min">0</div>
                                                        <div class="max"><?= $userBonuses->getClientBonuses()->maxBonusOut; ?></div>
                                                    </div>
                                                </div>
	...
	<div style="display: none">
		...
		$APPLICATION->IncludeComponent("bitrix:sale.location.selector.steps", "template2", Array(
