	...
	BX.Sale.OrderAjaxComponent = {
		...
		refreshOrder: function(result)
		{
			...
			else
			{
				...
				let allowBonus = result.order.IS_BONUS_ACTIVE == 'Y';
				 if(allowBonus) {
					 $('.order-bonuses').show();
				 } else {
					 $('.order-bonuses').hide();
				...
				if(this.orderBlockNode.querySelector('#orderBonusesVal')) {
					this.orderBlockNode.querySelector('#orderBonusesVal').value = result.order.BONUS_OUT
				}
				if(this.orderBlockNode.querySelector('#soa-property-81')){
					this.orderBlockNode.querySelector('#soa-property-81').value = result.order.BONUS_OUT
				}

				if(this.orderBlockNode.querySelector('#orderBonusesRange')) {
					this.orderBlockNode.querySelector('#orderBonusesRange').value = result.order.BONUS_OUT
				}
		...
		getYClientID: function() {
        	var match = document.cookie.match('(?:^|;)\\s*_ym_uid=([^;]*)');
			BX('soa-property-75').value = (match ? decodeURIComponent(match[1]) : '');
    	},
		...
		showDeliveryItemsPage: function(page)
		{
			...
			for (k = 0; k < this.deliveryPagination.currentPage.length; k++)
			{
				...
				if (this.deliveryPagination.currentPage[k].ID == 160) dpd = true;
			}
			for (k = 0; k < this.deliveryPagination.currentPage.length; k++)
			{
				deliveryItemNode = this.createDeliveryItem(this.deliveryPagination.currentPage[k], dpd);
		...
		bindEvents: function()
		{
			...
			BX.bind(this.orderBlockNode.querySelector('#orderBonusesVal'), 'change',  BX.proxy(this.bonusInputChange, this) )
			BX.bind(this.orderBlockNode.querySelector('#orderBonusesRange'), 'change',  BX.proxy(this.bonusRangeChange, this) )

			console.log('bind ', this.orderBlockNode.querySelector('#orderBonusesVal'), );
			BX.addCustomEvent('onDeliveryExtraServiceValueChange', BX.proxy(this.sendRequest, this));
		},

		bonusRangeChange: function(){
			this.orderBlockNode.querySelector('#orderBonusesVal').value = this.orderBlockNode.querySelector('#orderBonusesRange').value
			this.bonusInputChange()
		},

		bonusInputChange: function(){
			var valBonuses = parseInt(this.orderBlockNode.querySelector('#orderBonusesVal').value);

			this.orderBlockNode.querySelector('#orderBonusesRange').value = valBonuses;
			if(this.orderBlockNode.querySelector('#soa-property-81')) {
				this.orderBlockNode.querySelector('#soa-property-81').value = valBonuses;
			}
		...
		createDeliveryItem: function(item, dpd)
		{
			if (location.search.indexOf('get_dpd') !== -1 && dpd) {
				if (item.ID == 160) {
					var checked = 1;
				} else {
					var checked = 0;
		...
		alterProperty: function(settings, propContainer)
		{
			...
			if (textNode)
			{
				...
				if (settings.IS_PHONE == 'Y')
					textNode.setAttribute('data-maska', '+7 (9##) ### ## ##');
		...
		editTotalBlock: function()
		{
			...
			{
				this.totalInfoBlockNode.appendChild(this.createTotalUnit(BX.message('SOA_SUM_WEIGHT_SUM'), total.ORDER_WEIGHT_FORMATED)).id = 'totalWeight';
			}
			...
			/**  BONUSES **/
			if( parseInt(this.result.ADD_BONUSES) > 0  ) {
				this.totalInfoBlockNode.appendChild(this.createTotalUnit('Бонусы за заказ:', `<span class="sbonus-product"><span>${parseInt(this.result.ADD_BONUSES)}</span></span>` ));
			}
			...
			if( this.result.BONUS_OUT && parseInt(this.result.BONUS_OUT ) > 0 ) {
				this.totalInfoBlockNode.appendChild(this.createTotalUnit('Оплата бонусами:',  this.result.BONUS_OUT_FORMAT ));
			}
			...
			else
			{
				if( this.result.BONUS_OUT && parseInt(this.result.BONUS_OUT ) > 0 ) {
					...
					this.totalInfoBlockNode.appendChild(this.createTotalUnit('К оплате', total.ORDER_TOTAL_PRICE_SALE_FORMATED, {total: true}));
		...
		totalBlockScrollCheck: function()
		{
			...
			var bxpanelHeight = $('#panel').height(),
				headerwrapperHeight = $('.header-wrapper.header-v17').height(),
				bxSoaOrderHeight = $('#bx-soa-order').height(),
				bxSoaBasketHeight = $('#bx-soa-basket').height(),
				bsSoaOffset = (bxSoaOrderHeight - bxSoaBasketHeight),
				offsetTop = (bxpanelHeight + headerwrapperHeight + 25);
			...
			if (scrollTop > bsSoaOffset + 20) {
				...
				this.totalInfoBlockNode.style.position = 'absolute';
				this.totalInfoBlockNode.style.top = (bsSoaOffset - 154) + 'px';
			} else {
				BX.removeClass(this.totalInfoBlockNode, 'bx-soa-cart-total-bottom');
				this.totalInfoBlockNode.style.position = '';
				this.totalInfoBlockNode.style.top = (90 + bxpanelHeight) + 'px';
			}
