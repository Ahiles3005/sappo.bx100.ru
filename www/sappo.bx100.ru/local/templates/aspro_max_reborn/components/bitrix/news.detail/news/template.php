			...
			<div class="introtext">
				...
				<?else:?>
					...
					<div class="toggle-arrow-container">
						<div class="toggle-arrow arrow-down"></div>
					</div>
				<?endif;?>
			</div>
		</div>
...
ob_start();
if (
	isset($arResult['PROPERTIES']['AUTHOR']['VALUE'])
	&& strlen($arResult['PROPERTIES']['AUTHOR']['VALUE'])
):?>
<div class="blog-article-author-block">
	<?= Loc::getMessage('T_BLOG_ARTICLE_AUTHOR') ?>: <?= $arResult['PROPERTIES']['AUTHOR']['VALUE']; ?>
</div>
<?php endif;
$htmlAuthor = ob_get_clean();
?>
<?php $this->SetViewTarget('AUTHOR_BLOCK'); ?>
<?= $htmlAuthor; ?>
<?php $this->EndViewTarget(); ?>
<?//end Author text block?>
...
<?// date active from or dates period active?>
	...
	<div class="period_wrapper_inner">
		...
		<div class="blog-panel">
			...
			<?endif;?>
			<div class="blog-panel-views">
				<?//=CMax::showIconSvg("sale", SITE_TEMPLATE_PATH.'/images/svg/eye.svg', '', '', true, false);?>
            ...
            <div class="vote-block">
                ...
                $APPLICATION->IncludeComponent(
                    ...
                    array(
                        ...
                        "SET_STATUS_404" => "N",
                        "DISPLAY_AS_RATING" => "vote_avg", // Отображать как рейтинг или среднее значение
                        "CACHE_TYPE" => "A",
                        "CACHE_TIME" => "3600"
                    ),
                    $component
...
<?if($arResult['DISPLAY_PROPERTIES_FORMATTED'] && !$bBannerChar && $arParams["SHOW_TOP_PROJECT_BLOCK"] != "Y" && $arParams["STAFF_MODE"] != "Y" && $arParams["SHOW_TOP_PARTNERS_BLOCK"] != "Y"):?>	
	...
	<div class="ordered-block wraps properties-block catalog_detail with-title">			
		<div class="ordered-block__title option-font-bold font_lg huelge"> 
...
<?if($arResult['PROPERTIES']['ANALOGS_BRAND']['ELEMENTS']){?>
<?$this->SetViewTarget('analog_brands');?>
    <div class="search-analog-cloud">
        <h2 class="analog_title font_lg"><?=Loc::getMessage('ANALOGS_BRAND');?></h2>
        <div class="analog">
            <?foreach($arResult['PROPERTIES']['ANALOGS_BRAND']['ELEMENTS'] as $arAnalog):?>
                <a href="<?=$arAnalog['DETAIL_PAGE_URL'];?>" rel="nofollow"><?=$arAnalog['NAME'];?></a>
            <?endforeach;?>
        </div>
    </div>
<?$this->EndViewTarget();?>
