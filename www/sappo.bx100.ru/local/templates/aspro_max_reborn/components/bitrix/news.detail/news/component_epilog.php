...
<?
if ($arParams["PARTNERS_MODE"] == "Y" && ($isHideLeftBlock || $APPLICATION->GetProperty(
            "HIDE_LEFT_BLOCK_DETAIL"
        ) == "Y")): ?>
...
foreach ($arBlockOrder as $code): ?>
    ...
    elseif ($code == 'docs' && $templateData['DOCUMENTS']): ?>
                        ...
                        <div class="col-md-3 col-sm-6 col-xs-12">
                            ...
                            <div class="file_type clearfix <?= $arItem["TYPE"]; ?>">
                                ...
                                <div class="description">
                                    <a href="<?= $arItem['SRC'] ?>" class="dark-color text"
                                       target="_blank"><?= $fileTitle ?></a>
        ...
        //gallery block?>
    <?
    elseif ($code == 'docs_brand' && $templateData['DOCUMENTS']): ?>
        <div class="wraps docs-block ordered-block with-title">
            <div class="ordered-block__title option-font-bold font_lg ">
                <?= ($arParams["T_DOCS"] ? $arParams["T_DOCS"] : GetMessage("T_DOCS")); ?>
            </div>
            <div class="docs_brand">
                <div class="docs_brand__flex">
                    <?
                    foreach ($templateData['DOCUMENTS'] as $index => $docID): ?>
                        <?
                        $arItem = CMax::GetFileInfo($docID); ?>
                        <div class="docs_brand__item bordered <?= ($index > 0 ? 'hidden-doc' : '') ?>">
                            <?
                            $fileName = substr($arItem['ORIGINAL_NAME'], 0, strrpos($arItem['ORIGINAL_NAME'], '.'));
                            $fileTitle = (strlen($arItem['DESCRIPTION']) ? $arItem['DESCRIPTION'] : $fileName);
                            ?>
                ...
                <?
                if (count($templateData['DOCUMENTS']) > 1): ?>
                    <button class="show-more-docs" onclick="toggleHiddenDocs()">Показать еще</button>
                <?
                endif; ?>
            </div>
        </div>
        ...
        <div class="wraps galerys-block swipeignore  muted777 ordered-block with-title">
                ...
                <div class="flexbox flexbox--row">
                    <div class="switch-item-block__count muted777 font_xs">
                        <div class="switch-item-block__count-wrapper switch-item-block__count-wrapper--small" <?= ($bShowSmallGallery ? "" : "style='display:none;'"); ?>>
                            <span class="switch-item-block__count-value"><?= count(
                                    $templateData['GALLERY_BIG']
                                ); ?></span>
                        ...
                        <div class="switch-item-block__count-wrapper switch-item-block__count-wrapper--big" <?= ($bShowSmallGallery ? "style='display:none;'" : ""); ?>>
                            <span class="switch-item-block__count-value">1/<?= count(
                                    $templateData['GALLERY_BIG']
                                ); ?></span>
                            <?= GetMessage('T_GALLERY_TITLE'); ?>
                            <span class="switch-item-block__count-separate">&mdash;</span>
                    ...
                    <div class="switch-item-block__icons-wrapper">
                        <span class="switch-item-block__icons<?= (!$bShowSmallGallery ? ' active' : ''); ?> switch-item-block__icons--big"
                              title="<?= GetMessage("BIG_GALLERY"); ?>"><?= CMax::showIconSvg(
                                "gallery",
                                SITE_TEMPLATE_PATH . "/images/svg/gallery_alone.svg",
                                "",
                                "colored_theme_hover_bg-el-svg",
                                true,
                                false
                            ); ?></span>
                        <span class="switch-item-block__icons<?= ($bShowSmallGallery ? ' active' : ''); ?> switch-item-block__icons--small"
                              title="<?= GetMessage("SMALL_GALLERY"); ?>"><?= CMax::showIconSvg(
                                "gallery",
                                SITE_TEMPLATE_PATH . "/images/svg/gallery_list.svg",
                                "",
                                "colored_theme_hover_bg-el-svg",
                                true,
                                false
            ...
            <div class="big-gallery-block "<?= ($bShowSmallGallery ? ' style="display:none;"' : ''); ?> >
                <div class="owl-carousel owl-theme owl-bg-nav short-nav" data-slider="content-detail-gallery__slider"
                     data-plugin-options='{"items": "1", "autoplay" : false, "autoplayTimeout" : "3000", "smartSpeed":1000, "dots": true, "nav": true, "loop": false, "rewind":true, "index": true, "margin": 10}'>
                    <?
                            ...
                            <div class="col-md-3 col-sm-4 col-xs-6">
                                <div class="item">
                                    <div class="wrap"><a href="<?= $arPhoto['DETAIL']['SRC'] ?>" class="fancy"
                                                         ...
                                                         title="<?= $arPhoto['TITLE'] ?>">
                                            <img data-src="<?= $arPhoto['PREVIEW']['src'] ?>"
                                                 src="<?= \Aspro\Functions\CAsproMax::showBlankImg(
                                                     $arPhoto['PREVIEW']['src']
                                                 ); ?>" class="lazy img-responsive inline"
                                                 title="<?= $arPhoto['TITLE'] ?>" alt="<?= $arPhoto['ALT'] ?>"/></a>
                                    </div>
                                </div>
                            </div>
                        <?
    ...
    elseif ($code == 'goods'): ?>
        <?
        if ((in_array('LINK_GOODS', $arParams['PROPERTY_CODE']) || ($arParams['SHOW_LINKED_PRODUCTS'] == 'Y' && strlen(
                        $arParams['LINKED_PRODUCTS_PROPERTY']
                    )))
            ...
            if ($filter_by_filter) {
                if (!empty($arParams["~CONTENT_LINKED_FILTER_BY_FILTER"])) {
                    try {
                        // Декодируем JSON фильтра
                        $filterData = \Bitrix\Main\Web\Json::decode($arParams["~CONTENT_LINKED_FILTER_BY_FILTER"]);
                        ...
                        $productIds = [];
                        if (isset($filterData['CHILDREN'][0]['DATA']['value']) && is_array(
                                $filterData['CHILDREN'][0]['DATA']['value']
                            )) {
                            $productIds = array_map('intval', $filterData['CHILDREN'][0]['DATA']['value']);
                            $productIds = array_unique(array_filter($productIds));
                        }

                        if (!empty($productIds)) {
                            // Получаем первый склад из параметров
                            $storeId = !empty($arParams["STORES"]) ? (int)array_values($arParams["STORES"])[0] : 0;

                            if ($storeId <= 0) {
                                throw new Exception('Invalid store ID');
                            }

                            // 1. Сначала получаем ВСЕ товары и их разделы (независимо от наличия)
                            $allProductsData = [];
                            ...
                            while ($ob = $res->GetNext()) {
                                $allProductsData[(int)$ob['ID']] = [
                                    'SECTION_ID' => (int)$ob['IBLOCK_SECTION_ID']
                                ];
                            }

                            // 2. Теперь проверяем наличие на складе только тех товаров, которые существуют
                            $availableProducts = [];
                            if (!empty($allProductsData)) {
                                $res = CIBlockElement::GetList(
                                    ['ID' => 'ASC'],
                                    [
                                        'ID' => array_keys($allProductsData),
                                        'ACTIVE' => 'Y',
                                        'ACTIVE_DATE' => 'Y',
                                        'CATALOG_AVAILABLE' => 'Y',
                                ...
                                while ($ob = $res->GetNext()) {
                                    $availableProducts[] = (int)$ob['ID'];
                                }
                            }

                            // 3. Находим отсутствующие товары
                            $unavailableProducts = array_diff(array_keys($allProductsData), $availableProducts);

                            // 4. Ищем замену для отсутствующих товаров
                            $replacements = [];
                            if (!empty($unavailableProducts)) {
                                foreach ($unavailableProducts as $unavailableId) {
                                    if (!empty($allProductsData[$unavailableId]['SECTION_ID'])) {
                                        $sectionId = $allProductsData[$unavailableId]['SECTION_ID'];

                                        $replacementRes = CIBlockElement::GetList(
                                            ['RAND' => 'ASC'],
                                            [
                                                'IBLOCK_SECTION_ID' => $sectionId,
                                                '!ID' => $productIds, // Исключаем оригинальные товары
                                                'ACTIVE' => 'Y',
                                                'ACTIVE_DATE' => 'Y',
                                                'CATALOG_AVAILABLE' => 'Y',
                                                '>CATALOG_STORE_AMOUNT_' . $storeId => 0
                                        ...
                                        if ($replacementOb = $replacementRes->GetNext()) {
                                            $replacements[$unavailableId] = (int)$replacementOb['ID'];
                                        }
                                    }
                                }
                            }

                            // 5. Обновляем список товаров при наличии замен
                            if (!empty($replacements)) {
                                $updatedProductIds = [];
                                foreach ($productIds as $id) {
                                    $updatedProductIds[] = isset($replacements[$id]) ? $replacements[$id] : $id;
                                }

                                // Обновляем данные фильтра
                                $filterData['CHILDREN'][0]['DATA']['value'] = $updatedProductIds;
                                $arParams["~CONTENT_LINKED_FILTER_BY_FILTER"] = \Bitrix\Main\Web\Json::encode(
                                    $filterData
                                );
                            }
                        }
            ...
            $arItems = CMaxCache::CIBLockElement_GetList(
                array(
                    'CACHE' => array(
                        "MULTI" => "Y",
                        "TAG" => CMaxCache::GetIBlockCacheTag($arTheme["CATALOG_IBLOCK_ID"]["VALUE"])
                    )
                ),
                $arItemsFilter,
                false,
            ...
            <div class="assoc-block js-load-block tabs_slider loader_circle content_linked_goods" data-sections-ids=""
                 data-block="assoc" data-file="<?= $APPLICATION->GetCurPage() ?>">
                <div class="stub"></div>
                <?
                if ($arParams['IS_AJAX_SECTIONS'] == "Y" || $arParams['FROM_AJAX'] == 'Y') {
                    ...
                    if ($arParams['SHOW_SECTIONS_FILTER'] != "N" && isset($arFilterSectionsIDs) && is_array(
                            $arFilterSectionsIDs
                        ) && count($arFilterSectionsIDs) > 0) {
    ...
    elseif ($code == 'goods_nj'): ?>
                ...
                try {
                    ...
                    if (!empty($productIds)) {
                        ...
                        if (!empty($replacements)) {
                            ...
                            // Обновляем данные фильтра
                            $filterData['CHILDREN'][0]['DATA']['value'] = $updatedProductIds;
                            $arParams["~CONTENT_LINKED_FILTER_BY_FILTER"] = \Bitrix\Main\Web\Json::encode($filterData);
                        }
                    }
                } catch (Exception $e) {
                }
        ...
        <div class="assoc-block js-load-block_section tabs_slider loader_circle content_linked_goods" data-block="assoc"
    ...
    elseif ($code == 'goods_catalog'): ?>
        ...
        if ((in_array('LINK_GOODS', $arParams['PROPERTY_CODE']) || ($arParams['SHOW_LINKED_PRODUCTS'] == 'Y' && strlen(
            ...
            <?
            $APPLICATION->ShowViewContent('tags_page') ?>
            <?
            ...
            <?
            $APPLICATION->ShowViewContent('category_page') ?>
            <?
            $APPLICATION->ShowViewContent('analog_brands'); ?>
        <?
        endif; ?>
    <?
    endif; ?>
...
<?
$APPLICATION->IncludeFile('/include/microscheme/blog.php', $arResult, []); ?>