...
if (!empty($arResult['ITEMS'])){
    ...
    foreach ($arResult['ITEMS'] as $key => $arItem) {
        ...
        while ($arStoreAmount = $rsStoreAmount->Fetch()) {
            if ($arStoreAmount["STORE_ID"] === "5") {
                $arItem["Q_SPB"] = $arStoreAmount["AMOUNT"];
            }
            if ($arStoreAmount["STORE_ID"] === "3") {
                $arItem["Q_MSK"] = $arStoreAmount["AMOUNT"];
            }
        }
		...
		$arItem['GALLERY'] = CMax::getSliderForItemExt($arItem, $arParams['ADD_PICT_PROP'], 'Y' == $arParams['ADD_DETAIL_TO_SLIDER'], false);
		array_splice($arItem['GALLERY'], $arParams['MAX_GALLERY_ITEMS']);

		if ($arItem['CATALOG'] && !empty($arItem['OFFERS']))
		{
...
while ($getPrices = $resPrice->Fetch()) {
	$count++;
	if(!empty($getPrices['CATALOG_PRICE_2']) && $getPrices['CATALOG_PRICE_2']>0)
    $arPrices[] = intval($getPrices['CATALOG_PRICE_2']);
}
$minPrice = 0;
$maxPrice = 0;
if (is_array($arPrices)) {
	...
	$count=count($arPrices);
}
$images=[];
$counter=0;
foreach($arResult["ITEMS"] as $item){
	if($item["PREVIEW_PICTURE"]["SRC"] && $counter<3){
	$images[]='"'.$item["PREVIEW_PICTURE"]["SRC"].'"';
	$counter++;
	}elseif($item["DETAIL_PICTURE"]["SRC"]  && $counter<3){
	$images[]='"'.$item["DETAIL_PICTURE"]["SRC"].'"';
	$counter++;
	}
}
...
if (is_object($cp))
{
	...
	$cp->arResult['MIN_PRICE'] = $minPrice;
	$cp->arResult['MAX_PRICE'] = $maxPrice;
	$cp->arResult['NAME'] = (!empty($arResult["IPROPERTY_VALUES"]["SECTION_PAGE_TITLE"])?$arResult["IPROPERTY_VALUES"]["SECTION_PAGE_TITLE"]:$arResult["NAME"]);
	$cp->arResult['SECTION_PAGE_URL'] = $arResult["SECTION_PAGE_URL"];
	$cp->arResult["IMAGES"]=$images;
	$cp->arResult["COUNT"]=$count;
	$cp->arResult['DESCRIPTION'] = $arResult["DESCRIPTION"];
	$cp->SetResultCacheKeys(array('NAME','MIN_PRICE','MAX_PRICE','IMAGES','SECTION_PAGE_URL','DESCRIPTION','COUNT'));
