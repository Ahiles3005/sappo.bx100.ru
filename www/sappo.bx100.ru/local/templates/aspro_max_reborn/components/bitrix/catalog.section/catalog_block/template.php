    ...
    <div class="top_wrapper items_wrapper <?= $templateName; ?>_template <?= $arParams['IS_COMPACT_SLIDER'] ? 'compact-catalog-slider' : '' ?>">
        ...
        <div class="catalog_block items row <?= $arParams['IS_COMPACT_SLIDER'] ? 'swipeignore mobile-overflow' : '' ?> margin0 <?= $bHasBottomPager ? 'has-bottom-nav' : '' ?> js_append ajax_load block flexbox<?= ($bSlide ? ' owl-carousel owl-theme owl-bg-nav visible-nav short-nav hidden-dots swipeignore ' : ''); ?>"
            ...
            <? foreach ($arResult["ITEMS"] as $arItem) {
                ...
                $bonusesSum = $catalog->calculateBonus([ ['ID'=>$arItem['ID'], 'PRICE'=>$basePrice] ]);
                                        ...
                                        <div class="price_group min">
                                            <div class="price_matrix_wrapper">
                                                <div class="prices-wrapper">
                                                    <div class="price font-bold font_mlg font_mxs" data-currency=""
                                                         data-value="">
                                                    ...
                                                    <div class="sale_wrapper font_xxs">
                                                        ...
                                                        <div class="sale-number rounded2">
                                                            ...
                                                            <div class="inner-sale rounded1">
                                                                <div class="text">Экономия <span class="values_wrapper"><span
                    ...
                    <? if (!$arItem["OFFERS"]): ?>
                        <div class="counter_wrapp <?= ($arItem["OFFERS"] && $arParams["TYPE_SKU"] == "TYPE_1" ? 'woffers' : '') ?> clearfix rounded3">
                            ...
                            <div id="<?= $arItemIDs["ALL_ITEM_IDS"]['BASKET_ACTIONS']; ?>"
                                 class="button_block <?= ($arAddToBasketData["ACTION"] == "ORDER" || !$arAddToBasketData["CAN_BUY"] || !$arAddToBasketData["OPTIONS"]["USE_PRODUCT_QUANTITY_LIST"] || $arAddToBasketData["ACTION"] == "SUBSCRIBE" || $arAddToBasketData["ACTION"] == "MORE" ? "wide" : ""); ?>">
                                <!--noindex-->
                                <?= $arAddToBasketData["HTML"] ?>
                                <!--/noindex-->
                            </div>
                            ...
                            <div class="counter_wrapp ce_cmp_visible">
                                <div id="<?= $arItemIDs["ALL_ITEM_IDS"]['BASKET_ACTIONS']; ?>"
                                    ...
                                    <a class="btn btn-default basket read_more" rel="nofollow"
                                       data-item="<?= $arItem['ID'] ?>"
                                       href="<?= $arItem['DETAIL_PAGE_URL'] ?>"><?= Loc::getMessage('CATALOG_READ_MORE') ?></a>
                                </div>
                            </div>
                        </div>
                    <? if (isset($arCurrentSKU['PRICE_MATRIX']) && $arCurrentSKU['PRICE_MATRIX']) // USE_PRICE_COUNT
                ...
                <div class="col-lg-<?= $col; ?><? if ($bBigBlock): ?> col-md-8 col-sm-12 col-xs-12<? else: ?> col-md-4 col-sm-6 col-xs-6<? endif; ?> col-xxs-12 item item-parent js-notice-block item_block<?= ($bBigBlock ? ' big' : ''); ?> <?= ($arParams['SET_LINE_ELEMENT_COUNT'] ? 'custom-line' : ''); ?> catalog_element" id="<?= $arItem["ID"]; ?>">

                    <div class="sbonus-product v2"><span>+<?= round($bonusesSum[ $arItem['ID'] ]); ?></span></div>
                            ...
                            <? if ($bBigBlock): ?>
                                <? if ($bFonImg): ?>
                                    <a class="absolute-full-block lazy absolute-full-block_bg_center darken-bg-animate"
                                       href="<?= $arItem['DETAIL_PAGE_URL']; ?>" data-src="<?= $imageSrc ?>"
                                       data-bg="<?= $imageSrc ?>"
                                       style="background-image:url(<?= \Aspro\Functions\CAsproMax::showBlankImg($imageSrc); ?>)"><span></span></a>
                                <? endif; ?>
                            <? endif; ?>
                            <div id='<?= $totalCount <= 0 ? 'not_available' : '' ?>'
                                 class="image_wrapper_block js-notice-block__image <?= ($arParams['SHOW_PROPS'] == 'Y' && $arItem['DISPLAY_PROPERTIES'] && !$bBigBlock ? ' with-props' : ''); ?>">
...
<? endif; ?>
    <?if($arParams['SHOW_NG_SECTION']){?>
        <div class="text-center njbtn">
        <? if (!empty ($arResult['SECTION_DATA'])){ ?>
            <?
            foreach ($arResult['SECTION_DATA'] as $arItemSection){
            ?>
            <a class="btn btn-transparent-border-color btn-sm has-ripple" href="<?=$arItemSection['URL']?>"><?=$arItemSection['NAME']?></a>
            <?
            }
            ?>
        <?}else{?>
            <a class="btn btn-transparent-border-color btn-sm has-ripple" href="/catalog/">Перейти в каталог</a>
        <?}?>
        </div>
    <?}?>
    <script>
