...
<?if( count( $arResult["ITEMS"] ) >= 1 ){?>
	...
	<div class="table-view js_append flexbox flexbox--row<?=($bOptBuy ? ' with-opt-buy' : '');?>">
		...
		<?foreach($arResult["ITEMS"]  as $arItem){
			...
			<div class="table-view__item item js-notice-block bordered box-shadow main_item_wrapper" id="<?=$this->GetEditAreaId($arItem['ID']);?>" data-id="<?=$arItem["ID"]?>" data-product_type="<?=$arItem["CATALOG_TYPE"]?>">
				<div class="table-view__item-wrapper item_info catalog-adaptive flexbox flexbox--row">
					...
					<?//image-block?>
					<div class="item-foto">
						<div id='<?=$totalCount <= 0 ? 'not_available' : ''?>' class="item-foto__picture js-notice-block__image">
					...
					<?//text-block?>
					<div class="item-info">
						<div class="item-title"><a href="<?=$arItem["DETAIL_PAGE_URL"]?>" class="dark_link js-notice-block__title"><?=$elementName?></a></div>
							...
							<div class="item-actions flexbox flexbox--row">
								...
								<div class="item-price">
									<div class="cost prices clearfix">
										...
										$basePrice = $arItem["PRICES"]["Цена"]["DISCOUNT_VALUE"];
										$oldPrice = $arItem["PRICES"]["Цена без скидки"]["DISCOUNT_VALUE"];
																		...
																		<?ob_start();?>
																			<?if(($arAddToBasketData["OPTIONS"]["USE_PRODUCT_QUANTITY_LIST"] && $arAddToBasketData["ACTION"] == "ADD") && $arAddToBasketData["CAN_BUY"]):?>
																				...
																				<div class="counter_block<?=$class;?>" data-item="<?=$arItem["ID"];?>">
																					...
																					<input type="text" class="text" id="<? echo $arItemIDs["ALL_ITEM_IDS"]['QUANTITY']; ?>" name="<?=$arParams["PRODUCT_QUANTITY_VARIABLE"]; ?>" value="<?=floatval($cntBasketItems['QUANTITY']) ? floatval($cntBasketItems['QUANTITY']) : $arAddToBasketData["MIN_QUANTITY_BUY"]?>" />
