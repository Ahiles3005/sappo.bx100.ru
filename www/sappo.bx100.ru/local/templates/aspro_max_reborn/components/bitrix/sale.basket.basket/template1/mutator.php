...
foreach ($this->basketItems as $row)
{
    ...
    $bonusesSum = $catalog->calculateBonus([ ['ID'=>$row['PRODUCT_ID'], 'PRICE'=>$row['SUM_VALUE'], 'QUANTITY'=>$row['QUANTITY'] ] ], false, false);
	...
	$rowData = array(
		...
		'BONUSES' =>$bonusesSum[ $row['PRODUCT_ID'] ],
		'PROPS' => $row['PROPS'],
...
$sumBonus = $basket->calculateAllBonuses();
