...
BX.ready(function() {
	...
	if (window.location.href === "https://sappo.ru/test/favorites/") {
		let list = $('#basket-item-list');
    	list.find('.basket-items-list-item-container').not('.delayed-item').attr('id', 'inCart');
		$('.basket-checkout-container').hide()
	} else if (window.location.href === "https://sappo.ru/test/") {
		let list = $('#basket-item-list');
		list.find('.delayed-item').attr('id', 'delayedItem');
	}
});