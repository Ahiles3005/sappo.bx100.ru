...
BX.ready(function() {
	...
	if (window.location.pathname === "/basket/favorites/") {
		let list = $('#basket-item-list');
    	list.find('.basket-items-list-item-container').not('.delayed-item').addClass('inCart');
	} else if (window.location.pathname === "/basket/") {
		let list = $('#basket-item-list');
		list.find('.delayed-item').addClass('delayedItem');
	}
});