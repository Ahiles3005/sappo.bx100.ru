	...
	BX.Sale.BasketComponent = {
		...
		init: function(parameters)
		{
			BX.bind(
				...
				'click',
				BX.proxy(function() {
					for(let i of parameters.result.BASKET_ITEM_RENDER_DATA) {
						if (i.CURRENT_STORE_AMOUNT === '0') {
							this.actionPool.deleteItem(i.ID);
						}
					}
					setTimeout(() => {
		...
		showItemsCount: function()
		{
			...
			{
				let cnt = (parseInt(this.result.DELAYED_BASKET_ITEMS_COUNT))
					? (this.result.BASKET_ITEMS_COUNT - this.result.DELAYED_BASKET_ITEMS_COUNT)
					: this.result.BASKET_ITEMS_COUNT;

				itemCountNode.innerHTML = BX.message('SBB_IN_BASKET') + ' ' + cnt + ' ' + this.getGoodsMessage(this.result.BASKET_ITEMS_COUNT);
			...
			if (result.BASKET_ITEM_RENDER_DATA)
			{
				...
				for (i in result.BASKET_ITEM_RENDER_DATA)
				{
					if (result.BASKET_ITEM_RENDER_DATA.hasOwnProperty(i))
					{
						...
						let dataMax = $(`#basket-item-${newData.ID}`).find('.basket-item-amount-btn-plus').attr('data-max');
						if (!newData.CURRENT_STORE_AMOUNT) newData.CURRENT_STORE_AMOUNT = dataMax;
						if (newData.QUANTITY == newData.CURRENT_STORE_AMOUNT) newData.PLUS_DISABLE = 'disable';
						if (newData.QUANTITY == 1) newData.MINUS_DISABLE = 'disable';
		...
		getCorrectQuantity: function(itemData, quantity)
		{
			...
			if (quantity > itemData.CURRENT_STORE_AMOUNT)
			{
				quantity = itemData.CURRENT_STORE_AMOUNT;
			}
