  ...
  function setViewedProduct(id, arData) {
    try {
      ...
      if (typeof BX.localStorage !== "undefined" && typeof id !== "undefined" && typeof arData !== "undefined") {
        var item = typeof arData.item !== "undefined" ? arData.item : id;
        ...
        // delete item if other item (offer) of that item is exists
        if (typeof arViewedLocal[item] !== "undefined") {
          if (arViewedLocal[item].ID != id) {
            delete arViewedLocal[item];
            delete arViewedCookie[item];
          }
        }
        ...
        arData.IS_LAST = true;
        arViewedLocal[item] = arData;
        arViewedCookie[item] = [time.toString(), arData.PICTURE_ID];
        ...
        if (data.BASKET) {
          for (var i in data.BASKET) {
            ...
            if (typeof id === "number" || typeof id === "string") {
              $(".to-cart[data-item=" + id + "]").hide();
              $(".counter_block[data-item=" + id + "]").addClass('in');
              if ($(".counter_block[data-item=" + id + "] input").val() == $(".counter_block[data-item=" + id + "] .plus").data("max")) {
                $(".counter_block[data-item=" + id + "] .plus").addClass("disabled")
              }
  ...
  function viewItemCounter(id, price_id) {
    if (checkCounters()) {
      $.ajax({
        ...
        dataType: "json",
        type: "POST",
        data: { item: id, PRICE_ID: price_id },
        success: function (item) {
          if (item.ID) {
            waitLayer(100, function () {
              dataLayer.push({
...
$(document).click(function(e) {
  ...
  menu = _this.find('#new_menu'),
  menuBtn = _this.find('#menuButton'),
  container = $(".menu_fixed tbody tr td div.dropdown-menu.long-menu-items.BRANDS"),
  container2 = $("td.menu-item.dropdown"),
  bDarkness = $('.wrapper1.dark-hover-overlay').length > 0;

  if(e.target!=container[0]&&!container.has(e.target).length&&e.target!=container2[0]&&!container2.has(e.target).length) {
...
$(document).on('click', '.menu .mega-menu table td #menuButton, .menu-row .mega-menu table td #menuButton', function(e){
...
CheckHeaderFixed = function(){
	var header = $('header .header-wrapper .logo_and_menu-row.longs, body.simple_basket_mode #header').first(),
    ...
    $this.appear(
      function () {
        if ($calculateSpan.length) {
          ...
          var productId = $calculateSpan.attr("data-param-item") * 1;
...
$(document).ready(function () {
  ...
  $(document).on("click", ".counter_block:not(.basket) .plus", function () {
    let th = $(this).closest(".js-offers-calc").length
      ? $(this).closest(".js-offers-calc").find(".to-cart").first()
      : $(this).closest(".js-notice-block").find(".to-cart").first(),
    ...
    if (!$(this).parents(".basket_wrapp").length || $(this).parents(".services_in_basket").length) {
      if ($(this).parent().data("offers") != "Y") {
        ...
        if ($(this).parents(".counter_wrapp").length) {
          ...
          (tmp_ratio = !isDetailSKU2
            ? $(this).parents(".icons-basket-wrapper").find(".to-cart").data("ratio")
            : $(this).parents("tr").first().find("td.buy .to-cart").data("ratio")),
            (isDblQuantity = !isDetailSKU2
              ? $(this).parents(".icons-basket-wrapper").find(".to-cart").data("float_ratio")
              : $(this).parents("tr").first().find("td.buy .to-cart").data("float_ratio")),
            (ratio = isDblQuantity ? parseFloat(tmp_ratio) : parseInt(tmp_ratio, 10)),
            (max_value = "");
        }
        ...
        if (parseFloat($(this).data("max")) > 0) {
          if (input.val() < $(this).data("max")) {
            if (curValue <= $(this).data("max")) {
              $.ajax({
                type: "POST",
                url: arAsproOptions["SITE_DIR"] + "ajax/itemUpdate.php",
                data: { item, quantity: curValue, update_item },
                dataType: "json",
                success: function (data) {
                  ...
                  if (data !== null) {
                    if ("STATUS" in data) {
                        if ($("#ajax_basket").length) reloadTopBasket("update", $("#ajax_basket"), 200, 5000);
    ...
    if (!$(this).parents(".basket_wrapp").length || $(this).parents(".services_in_basket").length) {
      if ($(this).parent().data("offers") != "Y") {
        ...
        if (parseFloat($(this).parents(".counter_block").find(".plus").data("max")) > 0) {
          if (curValue < ratio) {
            arStatusBasketAspro = {};
            $.post( arMaxOptions['SITE_DIR']+'ajax/item.php', 'delete_item=Y&item='+item, $.proxy(function(){
              $('.to-cart[data-item='+item+']').removeClass('clicked');
              $('.to-cart[data-item='+item+']').attr("data-quantity", 1);
              $('.to-cart[data-item='+item+']').closest(".js-offers-calc").length
                ? $('.to-cart[data-item='+item+']').closest(".js-offers-calc").find('input[type=text]').val(1)
                : $('.to-cart[data-item='+item+']').closest(".js-notice-block").find('input[type=text]').val(1)
              $('.counter_block[data-item='+item+']').removeClass('in');
              $('.counter_block[data-item='+item+']').find('.plus').removeClass('disabled');
  ...
  $(document).on("click", ".to-cart:not(.read_more), .basket_item_add", function (e) {
    ...
    if (!th.hasClass("clicked")) {
      ...
      if (bIsService) {
        ...
        fill_prop["prop[ASPRO_BUY_item]"] = parent_product;
      }
              ...
              if ("STATUS" in data) {
                ...
                if (data.STATUS === "OK") {
                  ...
                  $(".to-cart[data-item=" + item + "]")
                    .closest(".button_block")
                    .addClass("wide");
                  $(".counter_block[data-item=" + item + "]").addClass("in")

                  if($(".counter_block[data-item=" + item + "]").find("input").val() == $(".counter_block[data-item=" + item + "]").find(".plus").data("max")) {
                    $(".counter_block[data-item=" + item + "]").find(".plus").addClass("disabled")
                  }
      ...
      if (!$(this).hasClass("text")) {
        if ($(this).hasClass("added")) {
          ...
          if (!bBannerAction) {
            ...
            $(this).closest(".main_item_wrapper").find(".counter_block").removeClass("in");
            $(this).closest(".main_item_wrapper").find('.to-cart').attr("data-quantity", 1);
            $(this).closest(".main_item_wrapper").find('input[type=text]').val(1);
            $(this).closest(".main_item_wrapper").find('.plus').removeClass('disabled');
            $(this).closest(".product-container").find(".counter_block").removeClass("in");
            $(this).closest(".product-container").find('.to-cart').attr("data-quantity", 1);
            $(this).closest(".product-container").find('input[type=text]').val(1);
            $(this).closest(".product-container").find('.plus').removeClass('disabled');
          }
  ...
  */
  ...
  $(document).on("click", ".ajax_load_btn", function () {
    var _this = $(this),
      ...
      var str_sort = _this.find('.more_text_ajax').data('sort');
      ...
      if ("clear_cache" in objUrl) {
        if (objUrl.clear_cache == "Y") add_url = "&clear_cache=Y&"+str_sort;
      } else {
        add_url = "&"+str_sort;
      }
      ...
      if ("BASKET" in arBasketAspro) {
        ...
        if (arBasketAspro.BASKET) {
          for (var i in arBasketAspro.BASKET) {
            ...
            $(".counter_block[data-item=" + i + "]").addClass("in");
            $(".in-cart[data-item=" + i + "]")
            .closest(".button_block")
            .addClass("wide");
            if ($(".counter_block[data-item=" + i + "] input").val() == $(".counter_block[data-item=" + i + "] .plus").data("max")) {
              $(".counter_block[data-item=" + i + "] .plus").addClass("disabled")
            }
        ...
        if (typeof BX.Sale.BasketComponent !== "undefined" && typeof BX.Sale.BasketComponent === "object") {
          $(document).on("click", ".basket-item-actions-remove", function () {
            ...
            if (BX.Sale.BasketComponent.items && BX.Sale.BasketComponent.items[basketID]) {
              delFromBasketCounter(BX.Sale.BasketComponent.items[basketID].item);
            }
          });
        }
      }
...
window.addEventListener('load', () => {
  const menuTabs = document.querySelectorAll('.sappo-categories-menu__left-item');

  menuTabs[0].classList.add('active');

  menuTabs.forEach((t) => {
    t.addEventListener('mouseenter', () => {
      menuTabs.forEach((rt) => rt.classList.remove('active'));
...
document.addEventListener('DOMContentLoaded', function() {
    // Получаем UTM-метки из сессии с сервера
    fetch('/ajax/get_utm.php')
        ...
        .then(data => {
            ...
            utmParams.forEach(function(param) {
                if (data[param]) {
                    utmString += param + '=' + encodeURIComponent(data[param]) + '&';
                }
            });

            if (utmString) {
                utmString = utmString.slice(0, -1); // Удаляем последний символ "&"
