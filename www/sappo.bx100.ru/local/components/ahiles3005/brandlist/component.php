...
if ($this->StartResultCache(false, array('v11', ($arParams["CACHE_GROUPS"] ? $USER->GetGroups() : false)))) {
...
foreach ($arResult["ITEMS"] as $PID => $arItem) {
    foreach ($arItem["VALUES"] as $key => $ar) {
        ...
        $code = strtolower($arItem['CODE']) . '_is_' . $ar['URL_ID'];


        $fakeUrl = str_replace('filter/#SMART_FILTER_PATH#/apply', $code, $url);

        $arResult["ITEMS"][$PID]['VALUES'][$key]['URL'] = $fakeUrl;
    }
}
